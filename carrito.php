...
if(isset($_SESSION['login'])){
	...
	if((isset($_POST['cantidad']) && isset($_POST['variedad'])) || (isset($_GET['cantidad']) && isset($_GET['variedad']))){
		...
		try{
			...
			?>
			<div id="mensaje">
				<div id="titulo"><?php echo $e->getMessage(); ?></div>
			</div>
			<?php
			...
			<div id="mensaje-error">
				<div id="titulo"><?php echo $ex->getMessage(); ?></div>
			</div>
			<?php
		}		
	}else{
...
<html> 
<head>
<title>Carrito de compras | Tienda Online de Giamber Lubricantes | <?php echo getCategoriasLeyenda(); ?></title>
<meta charset="UTF-8" />
...
<div class="contenedor">
	<form>
		<input name="text" type="text"title="Ingrese palabras separadas por espacio y no por simbolos. Por ejemplo: motul 5w40 auto" placeholder="Ingrese palabras separadas por espacio y no por simbolos" pattern="[a-zA-ZàáâäãåąčćęèéêëėįìíîïłńòóôöõøùúûüųūÿýżźñçčšžÀÁÂÄÃÅĄĆČĖĘÈÉÊËÌÍÎÏĮŁŃÒÓÔÖÕØÙÚÛÜŲŪŸÝŻŹÑßÇŒÆČŠŽ∂ð ,'-]{2,48}" required />
	...
	<div class="lubricantes opciones-busqueda visible" >
		<a class="tipo descarga" href="pdf.php" download="">Descargar lista de productos</a>
	</div>
...
<div class="contenedor">
	...
	<?php }elseif(isset($_GET['p']) && d($_GET['p'])==3) {
			...
			$pedido->setTerminado($envio);
			enviarCorreo($de,'Nuevo pedido de '.$pedido->getUsuario()->getNombre(),'Se recibió un pedido nuevo',$pedido->getUsuario()->getCorreo());
						
			
			echo '<div id="mensaje">Tu pedido (#'.$pedido->getID().') fue enviado</div>
			<div id="aclaraciones">Tu pedido se está procesando. En las próximas 48 horas, recibirás la confirmación via correo electrónico. Recordá verificar la casilla Spam o correo no deseado si no te llega la notificación. También podés ver el estado en la sección "Mis pedidos".</div>';
