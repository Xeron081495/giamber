...
try{
	...
	if($pedido->estaPagado()){
		echo '<meta http-equiv="refresh" content="0; url=../operacion.php?t='.urlencode('Este pedido ya fue pagado').'">';
	}else{
		$pedido->setPagado();
		enviarCorreo($pedido->getUsuario()->getCorreo(),'Tu pedido está pago #'.$pedido->getID().' | lubricantesgiamber.com',armarMensaje($pedido),$de);

	}
	//redireccionar
	echo '<meta http-equiv="refresh" content="0; url=../operacion.php?t='.urlencode('Se agregó el pedido como pago').'">';
...
function armarMensaje($pedido){
	...
	$mensaje   = '
	<html>
	<head><title>Se registró como pago tu pedido #'.$pedido->getID().'</title></head>
