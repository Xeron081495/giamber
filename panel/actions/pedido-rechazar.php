...
try{	
	...
	if($pedido->estaEntregado()){
		...
		$pedido->setRechazado(1);		
		enviarCorreo($pedido->getUsuario()->getCorreo(),'Tu pedido fue rechazado #'.$pedido->getID().' | lubricantesgiamber.com',armarMensaje($pedido),$de);		
	}
	//redireccionar
	echo '<meta http-equiv="refresh" content="0; url=../operacion.php?t='.urlencode('Se rechazo el pedido'.$nombre.'').'">';
...
function armarMensaje($pedido){
	...
	$mensaje   = '
	<html>
	<head><title>Se rechazo tu pedido #'.$pedido->getID().'</title></head>
	<body><h1><img src="http://'.$_SERVER['HTTP_HOST'].'/img/logo.png" width="200px" /></h1>
	<hr>';
	
	
	$mensaje.='<b>Productos del pedido rechazado</b><br />';
