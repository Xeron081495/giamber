...
$columnas = array('Nombre Producto','Precio Min.','Precio May.','Caja','Stock','Oferta');
$names = array('nombre','precio_min','precio_may','caja','stock','oferta');
$ancho_principal = 30;
$cant = count($columnas);
...
foreach(getProductos() as $prod){
	foreach($prod->getVariedades() as $v){
		array_push($datos, array($prod->getNombre().' de '.$v->getEnvase(),$v->getPrecioBruto(),$v->getPrecioMayoristaBruto(),$v->getCantidadMayorista(),$v->getStock(),$v->getOferta(),$v->getID()));
	}
}
		...
		<article id="tabla">
			...
			foreach($datos as $dato){
				...
				for($i=0; $i<count($dato)-1; $i++){				
					...
					}elseif($names[$i%count($names)]=="stock"){
						echo '<div class="col" style="width: '.$ancho.'%;">
							<select name="'.$names[$i%count($names)].'-'.$dato[count($dato)-1].'" required>
								<option value="'.$dato[$i].'" >Opción actual ('.$dato[$i].')</option>
			...
			foreach($datos as $dato){
				...
				for($i=0; $i<count($dato)-1; $i++){				
					if($i==0){
						...
						echo '<div class="col c'.$j.'" style="width: '.$ancho.'%;">
							<select name="'.$names[$i%count($names)].'-'.$dato[count($dato)-1].'" required>
								<option value="'.$dato[$i].'" >Opción actual</option>
								<option value="0">No hay stock</option>
								<option value="1">Hay stock</option>
							</select>
							</div>';
					}else{
						echo '<div class="col c'.$j.'" style="width: '.$ancho.'%;"><input step="0.01" name="'.$names[$i%count($names)].'-'.$dato[count($dato)-1].'" type="number" value="'.$dato[$i].'" required></div>';						
					}
				}
				echo '</div>';
				$j++;
