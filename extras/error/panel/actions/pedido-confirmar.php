...
try{	
		...
		foreach($pedido->getArticulos() as $art){
			...
			if($cant_entregada==$cant_pedida){ //se entrega la totalidad del producto
				...
				if($anula>0){ //se cancela el resto					
					...
					if(!isset($pedido_aplazado)) $pedido_aplazado = $pedido->getUsuario()->CrearPedido();	
					$pedido_aplazado->agregarVariedad($variedad->getID(),$resto);
				}								
			}		
		}
		
		//poenmos el estado nuevo 
		if(count($aplazado)==0 && count($anulado)==0){
			//confirmado totalmente
			$pedido->setConfirmado(1);
		}elseif(count($aplazado)>0 || count($anulado)>0){
			//confirmado parcialemnte
			$pedido->setConfirmado(2);
			
			//ver si hay aplazados
			if(count($aplazado)>0 && isset($pedido_aplazado)){
				$pedido_aplazado->setConfirmado(3);
				$pedido_aplazado->setEnvio(3);
			}
	...
	//redireccionar
	echo '<meta http-equiv="refresh" content="0; url=../operacion.php?t='.urlencode('Se confirmo el pedido'.$nombre.'').'">';
