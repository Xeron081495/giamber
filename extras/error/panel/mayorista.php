...
$columnas = array('Nombre','correo','CUIT','Usuario','Contraseña','Estado');
$ancho_principal = 30;
$cant = count($columnas);
...
foreach(getMayoristas() as $usuario){
	if($usuario->estaAprobado())
		...
		$estado = 'No Aprobado';
	array_push($datos, array($usuario->getNombre(),$usuario->getCorreo(),$usuario->getCUIT(),$usuario->getUsuario().' ',$usuario->getClave().' ',$estado,$usuario->getCorreo()));
}
